...
// Implement a MST algorithm, using an already implemented priority queue ADT
Graph GraphMST(Graph g) {
    PQ q = PQNew();
    Graph g2 = GraphNew(g->nV);
    Edge e = {0, 0, 0};
    for (int i = 0; i < g->nV; i++) {
    ...
    while (g2->nE < (g2->nV)-1) {
        Edge lightest = PQExtract(q);
        GraphInsertEdge(g2, lightest);
        if (GraphHasCycle(g2)) {
            GraphRemoveEdge(g2, lightest.v, lightest.w);
        }
    }
    return g2;
}
