// Matthew O'Dea Lab 1 - z5413887. Sorts linkedlist of integers
...
IntList IntListSortedCopy(IntList l) {
	IntList sortedCopy = IntListNew();

	for (struct IntListNode *curr = l->first;
			curr != NULL; curr = curr->next) {
		if (sortedCopy->first == NULL) {
			IntListAppend(sortedCopy, curr->data);
			continue;
		}
		IntListInsertInOrder(sortedCopy, curr->data);
	}
	return sortedCopy;
}
